...
import com.nekiplay.hypixelcry.utils.data.ProfiledData;
import com.nekiplay.hypixelcry.utils.itemlist.ItemRepository;
import com.nekiplay.hypixelcry.utils.itemlist.PetInfo;
import com.nekiplay.hypixelcry.utils.itemlist.SkyblockItemRarity;
...
public class PetCache {
    private static final Path FILE = HypixelCry.CONFIG_DIR.resolve("pet_cache.json");
    ...
    private static void parsePet(ItemStack stack, boolean clicked) {
        ...
        if (ItemUtils.getItemId(stack).equals("PET") && !profileId.isEmpty()) {
            //I once hoped that all pets would have a petInfo field, but that turned out to be false ;(
            PetInfo petInfo = ItemUtils.getPetInfo(stack);
        ...
        if (matcher.matches()) {
            ...
            if (rarity != null && rarity != SkyblockItemRarity.UNKNOWN) {
                ...
                if (!stack.isEmpty()) {
                    ...
                    String type = !ItemUtils.getPetInfo(copied).isEmpty() ? ItemUtils.getPetInfo(copied).type() : name.toUpperCase(Locale.ENGLISH).replace(" ", "_");
