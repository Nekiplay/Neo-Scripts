...
object LuaCommand {
    ...
    private fun openScriptsFolder(source: FabricClientCommandSource) {
        ...
        try {
            ...
            source.sendFeedback(Component.literal("${Main.PREFIX}§aOpening scripts folder..."))
        } catch (e: Exception) {
            // Если xdg-open не найден (например, в минималистичных сборках Linux)
            if (os.contains("nix") || os.contains("nux")) {
                source.sendFeedback(Component.literal("${Main.PREFIX}§cCould not open folder. Path: §7$absolutePath"))
            } else {
                source.sendFeedback(Component.literal("${Main.PREFIX}§cError: ${e.message}"))
            }
            e.printStackTrace()
        }
    }
    ...
    private fun toggleScript(source: FabricClientCommandSource, name: String) {
        ...
        if (loadedInstance != null) {
            ...
            if (scriptFile.exists()) {
                ...
                source.sendFeedback(Component.literal("${Main.PREFIX}§cFile §e$name.lua §cnot found in scripts directory."))
            }
        }
    }
    ...
    private fun executeLuaFile(filename: String, source: FabricClientCommandSource) {
        ...
        if (!scriptsDir.exists()) {
            scriptsDir.mkdirs()
            source.sendFeedback(Component.literal(Main.PREFIX + "§cDirectory for scripts: ${scriptsDir.path}"))
            return
        }
        ...
        if (!scriptFile.exists()) {
            source.sendFeedback(Component.literal(Main.PREFIX + "§cScript ${scriptFile.name} not found"))
            return
        }
        ...
        try {
            ...
            source.sendFeedback(Component.literal(Main.PREFIX + "§cScript execution error: ${e.message}"))
            e.printStackTrace()
        }
    }
    ...
    private fun unloadLuaScript(filename: String, source: FabricClientCommandSource) {
        ...
        if (luaManager.unloadScript(scriptName)) {
            ...
            source.sendFeedback(Component.literal(Main.PREFIX + "§cScript '$scriptName' is not loaded or not found"))
        }
    }
    ...
    private fun listLuaFiles(source: FabricClientCommandSource) {
        ...
        if (!scriptsDir.exists() || scriptsDir.listFiles()?.isEmpty() != false) {
            source.sendFeedback(Component.literal(Main.PREFIX + "§7No scripts available. Create files in: ${scriptsDir.path}"))
            return
        }
        ...
        if (scriptFiles.isNullOrEmpty()) {
            source.sendFeedback(Component.literal(Main.PREFIX + "§7No .lua or .luac files in scripts directory"))
            return
        }

        source.sendFeedback(Component.literal(Main.PREFIX + "§6Available scripts:"))
        scriptFiles.forEach { file ->
    ...
    private fun listLoadedScripts(source: FabricClientCommandSource, targetName: String?) {
        ...
        if (targetName != null) {
            ...
            if (scriptsToDisplay.isEmpty()) {
                source.sendFeedback(Component.literal("${Main.PREFIX}§cScript '$targetName' is not loaded."))
                return
            }

            source.sendFeedback(Component.literal("${Main.PREFIX}§6Dependency tree for §a$targetName§6:"))
        ...
        source.sendFeedback(Component.literal("${Main.PREFIX}§6Loaded scripts §7(${loadedScripts.size}):"))
    ...
    private fun compileLuaScript(source: FabricClientCommandSource, name: String) {
        ...
        if (!scriptsDir.exists()) {
            source.sendFeedback(Component.literal("${Main.PREFIX}§cScripts directory does not exist."))
            return
        }
        ...
        if (!sourceFile.exists()) {
            source.sendFeedback(Component.literal("${Main.PREFIX}§cSource file §e${sourceFile.name} §cnot found."))
            return
        }
        ...
        try {
            ...
            source.sendFeedback(Component.literal("${Main.PREFIX}§aCompiled §e${sourceFile.name} §a→ §e${outputFile.name} §7(${outputFile.length()} bytes)"))
        } catch (e: Exception) {
            source.sendFeedback(Component.literal("${Main.PREFIX}§cCompilation error: ${e.message}"))
            e.printStackTrace()
        }
    }
