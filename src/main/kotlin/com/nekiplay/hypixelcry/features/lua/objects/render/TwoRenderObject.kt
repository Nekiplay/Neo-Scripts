        ...
        override fun call(table: LuaValue): LuaValue {
            if (table.istable() && context != null) {
                ...
                if (scale != 1.0f) {
                    ...
                    context.drawText(textRenderer, Text.literal(text), x.toInt(), y.toInt(), color, isShadow)
                }
            }
            return NIL
        }
        ...
        override fun call(table: LuaValue): LuaValue {
            if (table.istable() && context != null) {
                ...
                try {
                    ...
                    if (identifier != null) {
                        val renderLayerFunction = java.util.function.Function<Identifier, RenderLayer> { id ->
                            // Для UI текстур используем стандартный cutout слой
                            RenderLayer.getGuiTextured(identifier)
                        }

                        context.drawTexture(
                            renderLayerFunction, // Function<Identifier, RenderLayer>
                            identifier,          // Identifier sprite
                            x, y,               // int x, int y
                            u.toFloat(),        // float u
                            v.toFloat(),        // float v
