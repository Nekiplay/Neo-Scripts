...
import static com.nekiplay.neoscripts.Main.CONFIG_DIR;
...
public class NEURepoManager {
     ...
     * @see #NEU_REPO
     */
    private static final Path LOCAL_REPO_DIR = CONFIG_DIR.resolve("item-repo"); // TODO rename to NotEnoughUpdates-REPO
    /**
     * @see #isLoading()
     */
    ...
    private static CompletableFuture<Boolean> loadRepository() {
        return CompletableFuture.supplyAsync(() -> {
                ...
                if (Files.isDirectory(NEURepoManager.LOCAL_REPO_DIR)) {
                    try (Git localRepo = Git.open(NEURepoManager.LOCAL_REPO_DIR.toFile())) {
                        ...
                        if (result.isSuccessful()) {
                            LOGGER.info("[Neo Scripts NEU Repo] NEU Repository updated with merge status: {}", result.getMergeResult().getMergeStatus());
                        } else {
                            LOGGER.error("[Neo Scripts NEU Repo] Update failed with merge status: {}. Downloading new repository", result.getMergeResult().getMergeStatus());
                    ...
                    Git.cloneRepository()
                            ...
                            .call().close();
                    LOGGER.info("[Neo Scripts NEU Repo] NEU Repository Downloaded");
                }
            } catch (TransportException e) {
                LOGGER.error("[Neo Scripts NEU Repo] Transport operation failed. Most likely unable to connect to the remote NEU repo on github", e);
                success = false;
            } catch (Exception e) {
                LOGGER.error("[Neo Scripts NEU Repo] Encountered unknown exception while downloading NEU Repository", e);
                success = false;
            }
            ...
            try {
                ...
                LOGGER.error("[Neo Scripts NEU Repo] Encountered unknown exception while loading NEU Repository", e);
                success = false;
            }
            return success;
            ...
            CompletableFuture.allOf(afterLoadTasks.stream().map(CompletableFuture::runAsync).toArray(CompletableFuture[]::new)).exceptionally(e -> {
                LOGGER.error("[Neo Scripts NEU Repo] Encountered unknown exception while running after load tasks", e);
                return null;
            });
            return success;
        });
    ...
    private static void deleteAndDownloadRepositoryInternal(Player player) {
        ...
        REPO_LOADING = runner.apply(() -> {
            ...
            try {
                ...
                LOGGER.error("[Neo Scripts NEU Repo] Encountered unknown exception while deleting the NEU repo", e);
    ...
    public static CompletableFuture<Void> runAsyncAfterLoad(Runnable runnable) {
        return REPO_LOADING.thenRunAsync(runnable).exceptionally(e -> {
            LOGGER.error("[Neo Scripts NEU Repo] Encountered unknown exception while running after load task", e);
            return null;
