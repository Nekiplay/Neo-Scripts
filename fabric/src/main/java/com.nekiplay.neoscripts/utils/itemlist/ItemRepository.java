...
import com.nekiplay.neoscripts.utils.itemlist.recipes.SkyblockRecipe;
...
public class ItemRepository {
    ...
    public static Stream<SkyblockRecipe> getRecipes(ItemStack stack) {
        return NEURepoManager.getRecipes().getOrDefault(ItemUtils.getNeuId(stack), Set.of()).stream().map(ItemRepository::toSkyblockRecipe).filter(Objects::nonNull);
    }

    public static Stream<SkyblockRecipe> getUsages(ItemStack stack) {
        return NEURepoManager.getUsages().getOrDefault(ItemUtils.getNeuId(stack), Set.of()).stream().map(ItemRepository::toSkyblockRecipe).filter(Objects::nonNull);
    }
