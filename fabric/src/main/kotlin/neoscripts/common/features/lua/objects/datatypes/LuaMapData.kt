...
class LuaMapData(val mapData: MapItemSavedData) : LuaUserdata(mapData) {
    ...
    override fun get(key: LuaValue): LuaValue {
        return when (val field = key.tojstring()) {
            ...
            "tracking_position", "trackingPosition" -> valueOf(mapData.trackingPosition)
            "unlimited_tracking", "unlimitedTracking" -> valueOf(mapData.unlimitedTracking)
            "dimension" -> valueOf(mapData.dimension.identifier().toString())
            "center_x", "centerX" -> valueOf(mapData.centerX.toDouble())
            "center_z", "centerZ" -> valueOf(mapData.centerZ.toDouble())
            "is_exploration_map", "isExplorationMap" -> valueOf(mapData.isExplorationMap)
            "tracked_decoration_count", "trackedDecorationCount" -> valueOf(mapData.trackedDecorationCount.toDouble())
            "banners" -> getBanners()
            "decorations" -> getDecorations()
            "frame_markers", "frameMarkers" -> getFrameMarkers()
            "color_data", "colorData" -> getColorData()
            "colors" -> getColorsArray()
            "getColor" -> GetColor()
            "update_color", "updateColor" -> UpdateColor()
