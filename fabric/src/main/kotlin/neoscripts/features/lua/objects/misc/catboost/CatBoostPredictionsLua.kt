package com.nekiplay.neoscripts.features.lua.objects.misc.catboost
...
class CatBoostPredictionsLua(val predictions: CatBoostPredictions) : LuaUserdata(predictions) {
    ...
    inner class Get : VarArgFunction() {
        override fun invoke(args: Varargs): LuaValue {
            ...
            return valueOf(predictions.get(onjectIndex, predictionIndex))
        }
    }

    inner class GetObjectCount : ZeroArgFunction() {
        override fun call(): LuaValue? {
            return valueOf(predictions.objectCount)
        }
    }
}
