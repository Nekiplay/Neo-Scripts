package com.nekiplay.neoscripts.client.features.commands.impl
...
object LuaCommand {
    ...
    fun register(dispatcher: CommandDispatcher<FabricClientCommandSource>, registryAccess: CommandBuildContext) {
        val luaCommand = ClientCommands.literal("lua")
            ...
            .then(ClientCommands.literal("hwid")
                .executes { context ->
                    context.source.sendFeedback(Component.literal("${ClientMain.PREFIX}§7HWID: §a" + Utils.getHWID8()))
                    1
                }
            )
    ...
    private fun openScriptsFolder(source: FabricClientCommandSource) {
        ...
        try {
            ...
            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§aOpening scripts folder..."))
        } catch (e: Exception) {
            // Если xdg-open не найден (например, в минималистичных сборках Linux)
            if (os.contains("nix") || os.contains("nux")) {
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cCould not open folder. Path: §7$absolutePath"))
            } else {
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cError: ${e.message}"))
            }
            e.printStackTrace()
        }
    }
    ...
    private fun toggleScript(source: FabricClientCommandSource, name: String) {
        val luaManager = ClientMain.LUA_MANAGER
        ...
        if (loadedInstance != null) {
            ...
            try {
                ...
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§7Script §a${loadedInstance.scriptName} §7has been §cunloaded§7."))
            } catch (e: Exception) {
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cError unloading script: ${e.message}"))
            }
        } else {
            // Если скрипт не загружен — ЗАГРУЖАЕМ
            val scriptsDir = File("config/neoscripts/scripts")
            ...
            if (scriptFile.exists()) {
                try {
                    ...
                    if (script != null) {
                        ...
                        source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cFailed to load script §e$name§c: client not found"))
                    }
                } catch (e: Exception) {
                    source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cFailed to load script §e$name§c: ${e.message}"))
                }
            } else {
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cFile §e$name.lua §cnot found in scripts directory."))
            }
        }
    }
    ...
    private fun executeLuaFile(filename: String, source: FabricClientCommandSource) {
        ...
        if (!scriptsDir.exists()) {
            scriptsDir.mkdirs()
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§cDirectory for scripts: ${scriptsDir.path}"))
            return
        }
        ...
        if (!scriptFile.exists()) {
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§cScript ${scriptFile.name} not found"))
            return
        }
        ...
        try {
            ...
            if (script != null) {
                ...
                if (wasLoaded == true) {
                    ...
                    source.sendFeedback(Component.literal(ClientMain.PREFIX + "§aScript '${scriptFile.nameWithoutExtension}' executed successfully, result: '${result}'"))
                }
            }
            else {
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cFailed to load script §e${scriptFile.nameWithoutExtension}§c: client not found"))
            }
        } catch (e: Exception) {
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§cScript execution error: ${e.message}"))
            e.printStackTrace()
        }
    }
    ...
    private fun unloadLuaScript(filename: String, source: FabricClientCommandSource) {
        ...
        if (luaManager?.unloadScript(scriptName) == true) {
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§aScript '$scriptName' unloaded successfully"))


        } else {
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§cScript '$scriptName' is not loaded or not found"))
        }
    }
    ...
    private fun listLuaFiles(source: FabricClientCommandSource) {
        ...
        if (!scriptsDir.exists() || scriptsDir.listFiles()?.isEmpty() != false) {
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§7No scripts available. Create files in: ${scriptsDir.path}"))
            return
        }
        ...
        if (scriptFiles.isNullOrEmpty()) {
            source.sendFeedback(Component.literal(ClientMain.PREFIX + "§7No .lua or .luac files in scripts directory"))
            return
        }

        source.sendFeedback(Component.literal(ClientMain.PREFIX + "§6Available scripts:"))
        scriptFiles.forEach { file ->
    ...
    private fun listLoadedScripts(source: FabricClientCommandSource, targetName: String?) {
        ...
        if (loadedScripts.isEmpty()) {
            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§7No scripts currently loaded"))
            return
        }
        ...
        if (targetName != null) {
            ...
            if (scriptsToDisplay.isEmpty()) {
                source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cScript '$targetName' is not loaded."))
                return
            }

            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§6Dependency tree for §a$targetName§6:"))
        ...
        source.sendFeedback(Component.literal("${ClientMain.PREFIX}§6Loaded scripts §7(${loadedScripts.size}):"))
    ...
    private fun compileLuaScript(source: FabricClientCommandSource, name: String) {
        ...
        if (!scriptsDir.exists()) {
            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cScripts directory does not exist."))
            return
        }
        ...
        if (!sourceFile.exists()) {
            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cSource file §e${sourceFile.name} §cnot found."))
            return
        }
        ...
        try {
            ...
            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§aCompiled §e${sourceFile.name} §a→ §e${outputFile.name} §7(${outputFile.length()} bytes)"))
        } catch (e: Exception) {
            source.sendFeedback(Component.literal("${ClientMain.PREFIX}§cCompilation error: ${e.message}"))
            e.printStackTrace()
        }
    }
